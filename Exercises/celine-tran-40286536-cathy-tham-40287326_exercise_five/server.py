...
@app.route("/three")
def three():
    # three: Select all entries where the after_mood is positive
    # used the positive_moods helper array defined at the top
    results = mongo.db.dataStuff.find({
...
@app.route("/five")
def five():
    # five: Select entries that occur on Monday or Tuesday,
    # and sort them by event_affect_strength
    results = mongo.db.dataStuff.find({
...
@app.route("/six")
def six():
    # six: Select entries where BOTH start_mood and after_mood
    # are in the negative category, then sort by weather
    query = {
